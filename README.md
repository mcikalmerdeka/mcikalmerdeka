# Self Introduction and Profile Guidance

<!-- Header Animation -->
<div align="center">
  <img height="300" width="525" alt="GIF" src="https://raw.githubusercontent.com/mcikalmerdeka/assets/main/snorlax_animation.gif">
  <p style="font-size: 6px;"><small>Original creator: <a href="https://x.com/ring_hyacinth/status/1870386506776674376?t=X43CIzUItl7T6uabwSYIpQ&s=19" target="_blank">@ring_hyacinth on X (formerly Twitter)</a></small></p>
</div>

<br>

<!-- Main Profile Introduction -->
Hi, I'm Cikal. I am an AI Engineer and Data Professional specializing in LLM-based systems orchestration (Multi-agent workflows, retrieval-augmented generation, context engineering, etc.) and Analytics Engineering (Designing analytical data marts, building dashboards focusing on measurable business impact, etc.)

Nowadays I am more focused on AI Engineering where I have hands-on experience building production-ready RAG systems and integrating LLMs into full-stack applications with a strong focus on reliability, observability and scalability. But, I still train my data skills every week because I started my career with Data Analytics and Data Science path since at that time I wanted to experience the direct connection between technology and business impact and how data can be used to solve real-world problems and make data driven decisions.

My educational background is in Physics which really gave me a strong foundation about first principles problem solving approach along with analytical and experimental mindset that I always try to implement while doing technical and business analysis. A little bit of backstory, after my graduation in 2023, the news about AI breakthroughs such as OpenAI's GPT-4 and other generative AI models were all over the internet and it really kind of sparked my curiosity to try these applications myself and remembering back then I was really filled with excitement of seeing how these applications are very different from what technology I was used to. That's why I took a decision to pursue world of AI because I believe this will be the central focus of technology advancement in the future and I want to be a part of it.

In my freetime outside of working hours I enjoy reading and writing about the trending topics in AI and exploring new tools, frameworks, and models related to AI engineering and data science. I also like to do photography while walking around the town, read books like novel and manga, watching anime and movies, listen to music, and sometimes play online games especially fast paced online games like Dota 2 and CS2. These things does help me refresh my mind and make me more productive in working.

- 👨‍💻 All of my projects are available in my repositories. You can explore them through my **[Portfolio Website](https://mcikalmerdeka.vercel.app/)**
  
- 🌱 I’m actively exploring **[Natural Language Processing (NLP)](https://github.com/mcikalmerdeka/NLP-Learning)**

- 📫 How to reach me : **[Email](mailto:mcikalmerdeka@gmail.com)** | **[LinkedIn](https://www.linkedin.com/in/muhammad-cikal-merdeka-50a658266/)** | **[Discord](https://discordapp.com/users/699071814092980264)**

- 📜 My resume/CV : **[Updated Resume (JAN 2026)](https://drive.google.com/drive/folders/1mw9BBgjSsRgmJh3ofnmekU2o7Uap5hR6?usp=sharing)**

<!-- Profile Additional Info -->
<table>
  <tr>
    <td>
      <!-- Larger profile views -->
      <a href="https://github.com/mcikalmerdeka">
        <img src="https://komarev.com/ghpvc/?username=mcikalmerdeka&style=flat&color=1ED760" height="45"/>
      </a>
    </td>
    <td align="right">
      <!-- Spotify badges -->
      <a href="https://open.spotify.com/playlist/6kSi8yuu24PGF3UhcrZjn5?si=e31ef63f09234156">
        <img src="https://img.shields.io/badge/Spotify Playlist-EN-1ED760?style=for-the-badge&logo=spotify&logoColor=white"/>
      </a>
      <a href="https://open.spotify.com/playlist/7DupLwtWnoVS20uUVrpo62?si=1d76646dec8643df">
        <img src="https://img.shields.io/badge/JP-1ED760?style=for-the-badge&logoColor=white"/>
      </a>
      <a href="https://open.spotify.com/artist/5kVZa4lFUmAQlBogl1fkd6?si=0AkswLEfQV6jDpWCKPBRcA">
        <img src="https://img.shields.io/badge/Fav Artist-Aimyon-1ED760?style=for-the-badge&logo=spotify&logoColor=white"/>
      </a>
      <a href="https://open.spotify.com/artist/4UK2Lzi6fBfUi9rpDt6cik?si=YR_0a8SsQ3eE1GG3OOk-rg">
        <img src="https://img.shields.io/badge/Yorushika-1ED760?style=for-the-badge&logoColor=white"/>
      </a>
      <a href="https://open.spotify.com/artist/45ft4DyTCEJfQwTBHXpdhM?si=45P1YfNxR-mBbE9ANI1IDA-rg">
        <img src="https://img.shields.io/badge/Milet-1ED760?style=for-the-badge&logoColor=white"/>
      </a>
    </td>
  </tr>
</table>

<!-- Skillsets -->
## Languages | Libraries | Frameworks | Tools
![My Skills](https://go-skill-icons.vercel.app/api/icons?i=python,r,matlab,mysql,postgresql,sqlserver,sqlalchemy,sqlite,numpy,pandas,polars,scipy,tensorflow,pytorch,selenium,matplotlib,seaborn,sklearn,mlflow,prometheus,grafana,huggingface,streamlit,gradio,flask,fastapi,pydantic,n8n,agno,langchain,chatgpt,claude,gemini,ollama,azure,gcp,bigquery,snowflake,duckdb,pinecone,qdrant,supabase,tableau,pbi,metabase,looker,dbtlabs,airflow,docker,postman,pytest,excel,git,github,dbeaver,vscode,cursor,jupyter,googlecolab&theme=dark&titles=true&perline=15)

<!-- Github and Other Stats -->
## 📊 Stats

<div align="left">
  <img src="https://streak-stats.demolab.com?user=mcikalmerdeka&theme=dark" alt="GitHub Streak" />
</div>
